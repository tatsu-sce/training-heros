        ...
        const fetchStats = async () => {
            try {
                const { data, error } = await supabase
                    .from('profiles')
                    .select('is_present, last_check_in_at, student_id, avatar_stats, display_name, height, weight, body_fat, last_workout_at, fitness_goal')
                    .eq('id', user.id)
                    .single();
